...
public class AppConfig implements WebMvcConfigurer {
	...
	@Bean // Nécessaire pour que Spring créera automatiquement HibernateTemplate et en
	...
	public HibernateTemplate hibernateTemplate(final SessionFactory sessionFactory) {
		...
		// Injection de la session Factory reçue par autowired
		hibernateTemplate.setSessionFactory(sessionFactory);

		// Tracer la création de la template dans le journal
	...
	public DataSource getDataSource() {
		...
		dataSource.setUrl("jdbc:mysql://localhost:3306/gs_personnes_database"); // L'url d'accès à la base de données
	...
	// seule exception de type PersistenceExceptionTranslationPostProcessor
	// (embalage des execptions
	...
	// gestionnaire de transactions pour ne pas dépendre l'application (couche
	// service) avec
	...
	// Ajoute un intercepteur permettant de gérer la session Hibernate danns la
	// couche Web.
	// Ceci permet d'éviter les exceptions Lazy
